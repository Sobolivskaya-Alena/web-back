...
$getUid = isset($_GET['uid']) ? validinfo($_GET['uid']) : '';
...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  $name = (!empty($_COOKIE['name_error']) ? validinfo($_COOKIE['name_error']) : '');
  $number = (!empty($_COOKIE['number_error']) ? validinfo($_COOKIE['number_error']) : '');
  $email = (!empty($_COOKIE['email_error']) ? validinfo($_COOKIE['email_error']) : '');
  $data = (!empty($_COOKIE['data_error']) ? strtotime($_COOKIE['data_error']) : '');
  $radio = (!empty($_COOKIE['radio_error']) ? validinfo($_COOKIE['radio_error']) : '');
...
function setVal($enName, $param){
  global $values;
  $values[$enName] = empty($param) ? '' : validinfo($param);
}
  ...
  if (isset($_COOKIE['csrf_error'])) {
    $messages['error'] = 'Ошибка в CSRF токене';
  ...
  if (!empty($_COOKIE['save'])) {
    ...
    if (!empty($_COOKIE['password'])) {
      $messages['info'] = sprintf('Вы можете <a href="login.php">войти</a> с логином <strong>%s</strong>
        ...
        validinfo($_COOKIE['login']),
        validinfo($_COOKIE['password']));
    }
  }
  ...
  $name = (!empty($_POST['name']) ? validinfo($_POST['name']) : '');
  $number = (!empty($_POST['number']) ? validinfo($_POST['number']) : '');
  $email = (!empty($_POST['email']) ? validinfo($_POST['email']) : '');
  $data = (!empty($_POST['data']) ? validinfo($_POST['data']) : '');
  $radio = (!empty($_POST['radio']) ? validinfo($_POST['radio']) : '');
  ...
  if($_SESSION['csrf_token'] != $csrf_tokens){
    setcookie('csrf_error', '1', time() + 24 * 60 * 60); 
  ...
  if(!val_empty('lang', "Выберите хотя бы один язык", empty($lang))){
    ...
    try {
      ...
      foreach ($lang as $key => $value) {
        $dbLangs->bindValue(($key+1), validinfo($value));
      }
      $dbLangs->execute();
